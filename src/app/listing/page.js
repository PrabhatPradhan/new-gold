...
import { FaStar, FaCity, FaUtensils, FaMapMarkedAlt, FaCar } from 'react-icons/fa';
...
export default function PackagesPage() {
    ...
    <>
      ...
      <div className="md:flex   px-4 md:px-12 border   ">
        ...
        <div className="w-full md:w-1/4 md:sticky top-24 border-r bg-white h-fit md:max-h-screen overflow-y-auto">
          ...
          <div className={`${mobileFilterOpen ? 'block' : 'hidden'} md:block p-4 space-y-6`}>
            {/* Budget */}
            <div>
              ...
              {[4, 5].map((star) => (
                <div key={star} className="flex items-center space-x-2">
                  <input
                    ...
                    onChange={() => handleToggle(starFilter, setStarFilter, star)}
                  />
                  <label>{star}★</label>
                </div>
              ))}
              ...
              {['Manali', 'Jaisalmer'].map((city) => (
                <div key={city} className="flex items-center space-x-2">
                  <input
                    ...
                    onChange={() => handleToggle(cityFilter, setCityFilter, city)}
                  />
                  <label>{city}</label>
                </div>
              ))}
              ...
              {['Customizable', 'Group Package'].map((type) => (
                <button
                  ...
                  className={`px-2 py-1 border rounded-lg mr-2 mt-1 ${
                    typeFilter === type ? 'bg-blue-500 text-white' : 'bg-white text-black'
                  }`}
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="w-full h-[12rem] relative">
          ...
          <div className="absolute bottom-2 left-2 right-2 flex items-center justify-between">
            ...
            <span className="text-xs text-white flex items-center gap-1 bg-black/40 px-2 py-0.5 rounded-full">
              {data.rating} <FaStar className="text-yellow-400 text-sm" />
            </span>
          </div>
        </div>
    ...
    {filteredPackages.length === 0 && (
      <p className="text-gray-500 text-center col-span-full">No packages match your filter.</p>
    )}
  </div>
</div>
